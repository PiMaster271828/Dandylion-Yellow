...
.learnSetLoop ; loop over the learn set until we reach a move that is learnt at the current level or the end of the list
	...
	push hl ; save hl before the call because the function modifies it (The next 5 lines were added by G-Dubs to allow
    call .tryToLearn                                                  ; multiple moves to be learned at the same time)
    pop hl ; restore hl to continue the loop                         ; (Followed tutorial)
    jr .learnSetLoop
.tryToLearn
