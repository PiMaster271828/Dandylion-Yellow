...
.handleEnemy
	...
	ld a, [wMonHBaseSpeed]
	ld b, a
   ;srl b                        ; (effective (base speed/2))    ; removed by G-Dubs to fix Focus Energy
	ldh a, [hWhoseTurn]
	and a
	ld hl, wPlayerMovePower
	ld de, wPlayerBattleStatus2
...
.calcCriticalHitProbability
	...
	ld hl, HighCriticalMoves     ; table of high critical hit moves        c
.Loop
...
.noCarry
	...
	ld b, $ff
.SkipHighCritical                                                          ; added until here by G-Dubs to fix Focus Energy
	ld a, [de]
	bit GETTING_PUMPED, a        ; test for focus energy
  /*jr nz, .focusEnergyUsed      ; bug: using focus energy causes a shift to the right instead of left,
	                             ; resulting in 1/4 the usual crit chance  ; removed by G-Dubs to fix Focus Energy
  */jr z, .noFocusEnergyUsed
   ;sla b                        ; (effective (base speed/2)*2)            ; removed by G-Dubs to fix Focus Energy
	sla b                        ; (effective (base speed*2))              ; added by G-Dubs to fix Focus Energy
   ;jr nc, .noFocusEnergyUsed                                              ; removed by G-Dubs to fix Focus Energy
	jr nc, .focusEnergyUsed                                                ; added by G-Dubs to fix Focus Energy
	ld b, $ff                    ; cap at 255/256
	jr .noFocusEnergyUsed
.focusEnergyUsed
   ;srl b                                                                  ; removed by G-Dubs to fix Focus Energy
	sla b                        ; (effective ((base speed*2)*2))          ; added until here by G-Dubs to fix Focus Energy
	jr nc, .noFocusEnergyUsed
	ld b, $ff                    ; cap at 255/256                          ; added until here by G-Dubs to fix Focus Energy
.noFocusEnergyUsed
   /*ld hl, HighCriticalMoves     ; table of high critical hit moves       ; removed until here by G-Dubs to fix Focus Energy
.Loop
	ld a, [hli]                  ; read move from move table
	cp c                         ; does it match the move about to be used?
	jr z, .HighCritical          ; if so, the move about to be used is a high critical hit ratio move
...
.noCarry
	...
	jr nc, .SkipHighCritical
	ld b, $ff
.SkipHighCritical                                                          ; removed until here by G-Dubs to fix Focus Energy
...
; if the move type matches the "attacking type" and one of the defender's types matches the "defending type"
	...
	ldh [hMultiplier], a
	and a  ; cp NO_EFFECT      ; this line and below added by G-Dubs to fix a misreporting of type effectiveness
	jr z, .gotMultiplier
...
.nothalf
	...
	and $7f
	sla a
.gotMultiplier                  ; this line and above added by G-Dubs to fix a misreporting of type effectiveness
	add b
	ld [wDamageMultipliers], a
	xor a
	ldh [hMultiplicand], a
...
AIGetTypeEffectiveness:
	...
	ld a, EFFECTIVE ; bug: should be EFFECTIVE (10)   ; Line fixed by G-Dubs (Listed as a bug in comments)
